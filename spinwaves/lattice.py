...
class Lattice:
    ...
    def constructU(self, orientation: Union[None, tuple, np.ndarray]) -> np.ndarray:
        ...
        if orientation == None:
            ...
            hkl = orientation
            n = np.dot(self.B, hkl)
            U = funs.rotate( np.cross(n, [0,0,1]), funs.angle(n, [0,0,1]) )
            ...
            R1 = funs.rotate( np.cross(n1, [1,0,0]), funs.angle([1,0,0], n1) )

            # Find the angle necessary to put hkl2 in `xy` plane
            n3 = np.dot(R1, n2)
            beta2 = funs.angle(n3, [n3[0],n3[1],0]) * np.sign(-n3[2])
            R2 = funs.rotate( [1,0,0], beta2 )
